...
import { gitlabLifecycleBackendPlugin } from './plugin';
...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [gitlabLifecycleBackendPlugin],
    });

    await request(server).get('/api/gitlab-lifecycle-backend/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/gitlab-lifecycle-backend/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/gitlab-lifecycle-backend/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/gitlab-lifecycle-backend/todos/${createdTodoItem.id}`)
